...
const getRedirectUri = () => {
  ...
  // const baseUrl = process.env.REACT_APP_REDIRECT_URI || window.location.origin;
  const baseUrl = URL || window.location.origin;
  return `${baseUrl}/callback`;
};

const AUTH_URL = 
  `https://accounts.spotify.com/authorize?client_id=e257dc917f8640b5a9afe2f6e6ac1ef9&response_type=code&redirect_uri=${getRedirectUri()}&scope=user-library-read user-read-playback-state playlist-read-collaborative playlist-read-private user-modify-playback-state user-read-private user-read-email streaming user-top-read playlist-modify-public playlist-modify-private user-library-modify`;
...
export default function Login() {
  return (
    <Container
      ...
      style={{ minHeight: "100vh"  }}
    >
      <a className="btn btn-danger btn-lg color: '#fff'" href={AUTH_URL}>
        Login With Spotify
      </a>
    </Container>
  )
